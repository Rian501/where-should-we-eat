...
eatsApp.factory("SuggestionsFactory", function ($q, $http, GoogleCreds, FirebaseUrl) {
	...
	let keywordArr = ["mexican", "italian", "chinese"];
	// let keyword = "food";
	let type = 'restaurant';
...
// keywordsArr.map( function(each){
// 	return $http.get(
// 		`https://emlemproxy.herokuapp.com/api/places/nearbysearch/json?location=${userLat},${userLon}&radius=${radiusM}&opennow=true&type=${type}&keyword=${each}&keyword=food&key=${placesAPI2}`
// 	)
// })
return $q.all(promisesArr)
	// .then((resultsArr) => {
	// 	console.log("proms results", resultsArr);
	// 	resolve(resultsArr);
	// })
	// .catch((err) => {
	// 	console.log("Error in the new promise set!", err);
	// })
